...
class GoDaddyPaymentsGateway extends AbstractPaymentGateway
{
    ...
    public static function isActive() : bool
    {
        ...
        // bail if the site was created before 2021-06-01 at 4:30 EDT (UTC) and does not fall into 1/3 of the existing sites
        if (Configuration::get('godaddy.site.created') < 1622530800 && ManagedWooCommerceRepository::getXid() % 3 > 0) {
            return false;
        }

        // bail if not on the MWC non-reseller plan
        if (! ManagedWooCommerceRepository::hasEcommercePlan() || ManagedWooCommerceRepository::isReseller()) {
            return false;
        }
